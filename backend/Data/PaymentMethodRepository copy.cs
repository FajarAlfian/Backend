    ...
    public class PaymentMethodRepository : IPaymentMethodRepository
    {
        ...
        public async Task<List<PaymentMethod>> GetAllPaymentMethodAsync()
        {
            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                string queryString = @"
                    SELECT payment_method_id, payment_method_name, created_at, updated_at
        ...
        public async Task<int> CreatePaymentMethodAsync(PaymentMethod paymentMethod)
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                await connection.OpenAsync();
                string queryString = @"
                    INSERT INTO ms_payment_method (payment_method_name, created_at, updated_at)
                    VALUES (@payment_method_name, @created_at, @updated_at);
        ...
        public async Task<bool> UpdatePaymentMethodAsync(PaymentMethod paymentMethod)
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                string queryString = @"
                    ...
                    SET payment_method_name = @payment_method_name, updated_at = @updated_at
                    WHERE payment_method_id = @payment_method_id";
                using (var command = new MySqlCommand(queryString, connection))
                {
